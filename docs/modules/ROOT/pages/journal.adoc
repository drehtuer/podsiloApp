...
== 2026-09-13 (later) — CI never ran detekt, and its own header said it did

*Attempted:* run `detekt` over `main`, and add it to CI if it was missing.

*It was missing, and the way it was missing is the interesting part.* `ci.yml`'s header comment has
always read "`Eight checks in one job, in this order: yamllint, shellcheck, asciidoctor, ktlint,
detekt, Android lint, the Tier 1 unit tests, then both APK builds`". Seven of those eight had a step.
The only Gradle invocations in the file were `ktlintCheck`, `lint`, `coverage` and
`assembleDebug assembleRelease`.

So CLAUDE.md §12 asked every task to pass `./gradlew ktlintCheck detekt test`, and the badge that
looked like it enforced that answered a different question. A project-wide detekt regression could
have reached `main` green at any point. Nothing is known to have slipped through -- a full
`--rerun-tasks` over all twelve modules is clean -- but that is luck plus the per-module runs done by
hand, not the pipeline.

*Worth noticing:* this was found by reading the workflow to answer "`did CI already cover the thing I
could not run locally?`", which is a question worth asking before leaning on CI rather than after.
The answer was no, and the honest version of the previous session's summary depended on knowing it.

*The cost argument is one-sided,* which is why this is a step rather than a backlog entry. detekt
runs without type resolution, so it resolves no dependency and compiles nothing: six seconds for
every module, forced, against ten minutes for the test step it sits above. There was never a
performance reason for the omission; it looks like a step that was described and then not written.

*Verified:* `./gradlew detekt --rerun-tasks` on `main` -- twelve tasks executed, zero findings. The
first run reported `UP-TO-DATE` for everything from this session's earlier caches, which is not a
verification of anything; `--rerun-tasks` is what makes the green mean something.


== 2026-09-13 (release) — 0.7.2, one behaviour change and an honest count

*Attempted:* cut v0.7.2.

*What is actually in it, checked rather than assumed.* `git log v0.7.1..main` is four commits and
two merges, and `git diff v0.7.1..main -- '*/src/**'` is *two files*: `SyncOrchestrator.kt` and the
new `ServerDidNotKeepActionsTest.kt`. `gradle/libs.versions.toml` is untouched, so unlike 0.7.1
there are not even dependency bumps to list.

So the release is one sentence: *Podsilo now notices when Nextcloud accepts an episode action and
keeps nothing, and records it in the error log against the podcast it happened to.* The other merge
of the day -- CI actually running `detekt` -- does not reach the APK and is not release notes,
however much activity it represented.

*The trap this release procedure walks into is the README,* and this time it was the documentation
that was wrong rather than the person. `dev-environment.adoc` §10 step 1 still read "`Merge the
chore(release): X.Y.Z commit (version bump + README status line)`" -- but that status block was
deliberately removed at 0.7.1, with the 0.7.1 journal entry saying in terms "`Do not reintroduce it
at the next bump`". Following the documented procedure literally would have put it straight back.
Fixed in the same change as the bump, because a release procedure is only worth having if it is the
one actually followed.

*The immutable-release order still governs everything:* merge the bump, push the tag, wait for CI to
attach the APKs to the *draft*, then publish. Publishing first burns the version number for good --
that is how v0.5.0 was lost, and the tag stays reserved even after the release is deleted.
